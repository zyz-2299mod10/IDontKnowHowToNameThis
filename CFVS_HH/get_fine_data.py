...
custom_parameters = [ # --device,
    {"name": "--controller", "type": str, "default": "ik", "help": "Controller to use for Franka. Options only ik"},
    {"name": "--object", "type":str, "help": "collected object"},
    {"name": "--num_envs", "type": int, "default": 1, "help": "Number of environments to create"},
    {"name": "--chunk_idx", "type": int, "default": 0, "help": "chunk_idx"},
    {"name": "--asset_root", "type": str, "default": "/home/hcis/isaacgym/assets", "help": "Isaac Robot asset path"},
    {"name": "--urdf_root", "type": str, "default": "/home/hcis/YongZhe/CFVS_HH/obj-and-urdf/urdf", "help": "CAD model urdf path"}
]
...
# create usb place
usb_place = f'{args.object}_Male_place.urdf'
asset_options = gymapi.AssetOptions()
# asset_options.fix_base_link = True
...
for i in range(num_envs):
    ...
    usb_pose.p.z = table_dims.z * 0.5 + table_pose.p.z + 0.07 + np.random.uniform(-0.008, 0.012)
    ...
    r [-2pi, 2pi]
    '''
    usb_place_pose.p.x = usb_pose.p.x + np.random.uniform(-0.005, 0.005)
    usb_place_pose.p.y = usb_pose.p.y + np.random.uniform(-0.005, 0.005)
    ...
    usb_bottom_pose.p = gymapi.Vec3(usb_pose.p.x, usb_pose.p.y, usb_pose.p.z - 0.008)
    ...
    hole_top_pose.p = gymapi.Vec3(usb_place_pose.p.x, usb_place_pose.p.y, usb_place_pose.p.z + 0.057)
...
for env in range(num_envs):
    ...
    grasp_pos.append([grasp_position[0] + usb_pos_tmp[0],
                      ...
                      grasp_position[2] + usb_pos_tmp[2]])   
...
while not gym.query_viewer_has_closed(viewer):    
    ...
    grasp_offset = 0.11
    grasped_threshold = 0.021
    gripper_sep = dof_pos[:, 7] + dof_pos[:, 8]
    gripped = (gripper_sep < grasped_threshold)
    ...
    if IsGrasp.all():
        ...
        for env in range(num_envs):
            ...
            # Following staff are useless in coarse part. I tried to reduce the redundant, but his way to construct dataset it's too annoying... it will keep erroring if I do so. 
            # So I just keep it all. Sorry :P
            ...
            '3d_keypoint_camera_frame': [[float(v) for v in peg_top_in_cam],
